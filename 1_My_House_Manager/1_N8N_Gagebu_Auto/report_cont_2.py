    ...
    try:
        ...
        for building, rooms in room_config.items():
            ...
            for room in rooms:
                ...
                if not clean_names:
                    sheet_data[f"{room}호"] = pd.DataFrame(columns=gagebu_df.columns[:-1])
                    continue
                ...
                sheet_data[f"{room}호"] = room_history.drop(columns=['temp_name'])

            write_sheets_value_only(building_file, sheet_data)
